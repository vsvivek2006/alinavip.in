 ...
 * 
 * Usage:
 *   node scripts/verify-image-cdn.cjs <BASE_URL>
 */
...
if (!BASE_URL) {
  ...
  console.log('Usage: node scripts/verify-image-cdn.cjs <BASE_URL>');
  process.exit(1);
}
...
async function run() {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (res.statusCode === 200) {
      console.log(`✓ 200 OK (${res.contentType}, ${res.contentLength} bytes)`);
      passed++;
    } else {
