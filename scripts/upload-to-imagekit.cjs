 ...
 * 
 * Usage:
 *   node scripts/upload-to-imagekit.cjs <IMAGEKIT_PRIVATE_KEY> <IMAGEKIT_URL_ENDPOINT>
 */
...
const URL_ENDPOINT = (process.argv[3] || process.env.IMAGEKIT_URL_ENDPOINT || '').replace(/\/$/, '');

let ASSETS_DIR = path.resolve(__dirname, '../../Alina VIp Assets');
if (!fs.existsSync(ASSETS_DIR)) {
  ASSETS_DIR = path.resolve('D:/ALINA VIP/Alina VIp Assets');
}
if (!fs.existsSync(ASSETS_DIR)) {
  ASSETS_DIR = path.resolve(__dirname, '../public/images/assets');
}

if (!PRIVATE_KEY) {
  console.error('ERROR: ImageKit Private API Key required.');
  console.log('Usage: node scripts/upload-to-imagekit.cjs <IMAGEKIT_PRIVATE_KEY> [IMAGEKIT_URL_ENDPOINT]');
  process.exit(1);
}
...
async function run() {
  ...
  console.log(`CDN Base URL: ${URL_ENDPOINT}/shared/`);
  console.log(`========================================\n`);
}
